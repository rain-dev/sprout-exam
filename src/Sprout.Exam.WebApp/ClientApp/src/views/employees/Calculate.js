...
export class EmployeeCalculate extends Component {
  ...
  constructor(props) {
    super(props);
    this.state = { id: 0, fullName: '', birthdate: '', tin: '', salary: 0, typeId: 1, absentDays: 0, workedDays: 0, netIncome: 0, loading: true, loadingCalculate: false };
  }
  ...
  async calculateSalary() {
    ...
    const response = await fetch(`api/salary/calculate/${this.state.id}`, requestOptions);
    const data = await response.json();
    this.setState({ loadingCalculate: false, netIncome: data.netIncome });
  }
  ...
  async getEmployee(id) {
    ...
    if (response.status === 200) {
      const data = await response.json();
      this.setState({ id: data.id, fullName: data.fullName, salary: data.salary, birthdate: data.birthdate, tin: data.tin, typeId: data.typeId, loading: false, loadingCalculate: false });
    }
    else {
